...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint', type=str, default='files/checkpoints/Eruku_continuous_finetune_cond_drop_base')
    ...
    model = Emuru("google-t5/t5-large", 'blowing-up-groundhogs/emuru_vae')
    ...
    with torch.inference_mode():
        for idx, sample in enumerate(tqdm(dataset)):
            ...
            decoder_inputs_embeds_vae = model.get_model_inputs(img, img, img.size(-1), img.size(-1), 1024*1024)['decoder_inputs_embeds']

            result = model.generate(decoder_inputs_embeds_vae, [style_text], [gen_text], max_new_tokens=tgt_tokens)[0][:,:,:,img.size(-1):]
